        ...
        if (sender instanceof Player player) {
            if (args.length == 0) {
                if (getScoreboard().hasBoard(player)) {
                    ...
                    player.sendMessage(getMessage().addColor("Board has been disabled"));
                } else {
                    getScoreboard().apply(player);
                    player.sendMessage(getMessage().addColor("Board has been applied"));
                }
                return true;
                    ...
                    if (target != null) {
                        if (target == player) {
                            ...
                            if (getScoreboard().hasBoard(target)) {
                                ...
                                player.sendMessage(getMessage().addColor("&6Board has been disabled for&f " + target.getName()));
                            } else {
                                getScoreboard().apply(target);
                                player.sendMessage(getMessage().addColor("&6Board has been applied for&f " + target.getName()));
                            }
                        }
            ...
            if (args.length == 1) {
                ...
                if (target != null) {
                    if (getScoreboard().hasBoard(target)) {
                        getScoreboard().disable(target);
                        consoleCommandSender.sendMessage(getMessage().addColor("&6Board has been disabled for&f " + target.getName()));
                    } else {
                        getScoreboard().apply(target);
                        consoleCommandSender.sendMessage(getMessage().addColor("&6Board has been applied for&f " + target.getName()));
                    }
